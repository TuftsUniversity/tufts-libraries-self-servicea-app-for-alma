        ...
        mms_id_and_fund_dict.append(
            {
                ...
                "Transaction Type": transacation_item_type,
            }
        )
...
if not os.path.isdir(pDir) or not os.path.exists(pDir):
    os.makedirs(pDir)
mms_id_and_fund_df = pd.DataFrame(
    columns=["MMS Id", "Fund Ledger Code", "Fiscal Year", "Transaction Type"],
    data=mms_id_and_fund_dict,
)
...
for id in mms_id_list:
    ...
    one_hundred = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='100']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    one_hundred_e = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='100']/ns1:subfield[@code='e']", namespaces
    ):
    ...
    one_ten = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='110']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    one_ten_e = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='110']/ns1:subfield[@code='e']", namespaces
    ):
    ...
    seven_hundred = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='700']/ns1:subfield[@code='a']", namespaces
    ):
        seven_hundred.append(
            record.text.encode("utf-8", "ignore").decode("utf-8", "ignore")
        )

    # for element in seven_hundred:
    #     print(id + " - " + str(element.encode('utf-8', 'ignore').decode('utf-8', 'ignore')))
    outfile.write(
        ";".join(seven_hundred).encode("utf-8", "ignore").decode("utf-8", "ignore")
    )
    outfile.write("~")

    seven_hundred_e = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='700']/ns1:subfield[@code='e']", namespaces
    ):
    ...
    seven_ten = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='710']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    seven_ten_e = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='710']/ns1:subfield[@code='e']", namespaces
    ):
    ...
    six_fifty_five_a = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='655']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    two_forty_five_a = root_bib_record.find(
        ".//ns1:datafield[@tag='245']/ns1:subfield[@code='a']", namespaces
    )
    if two_forty_five_a is not None:
        # two_forty_fve_a = re.sub(r'().+?)\s$', r'\1', two_forty_five_a)
        outfile.write(two_forty_five_a.text)

    two_forty_five_b = root_bib_record.find(
        ".//ns1:datafield[@tag='245']/ns1:subfield[@code='b']", namespaces
    )
    ...
    two_sixty_a = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='260']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    two_sixty_b = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='260']/ns1:subfield[@code='b']", namespaces
    ):
    ...
    two_sixty_c = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='260']/ns1:subfield[@code='c']", namespaces
    ):
    ...
    two_sixty_four_a = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='264']/ns1:subfield[@code='a']", namespaces
    ):
    ...
    two_sixty_four_b = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='264']/ns1:subfield[@code='b']", namespaces
    ):
    ...
    two_sixty_four_c = []
    for record in root_bib_record.findall(
        ".//ns1:datafield[@tag='264']/ns1:subfield[@code='c']", namespaces
    ):
    ...
    while x < count:
        ...
        if gfSegment.iloc[x]["Second Author Name"] != "Empty":
            ...
            creator += parseCreator(
                secondAuthor, secondAuthorRelator, "personal", test_mms_id
            )
        ...
        if gfSegment.iloc[x]["Corporate Author Name"] != "Empty":
            ...
            creator += parseCreator(
                corporateAuthor, corporateAuthorRelator, "corporate", test_mms_id
            )
        ...
        if gfSegment.iloc[x]["Second Corporate Author Name"] != "Empty":
            ...
            secondCorporateAuthorRelator = gfSegment.iloc[x][
                "Second Corporate Author Relator"
            ]
            creator += parseCreator(
                secondCorporateAuthor,
                secondCorporateAuthorRelator,
                "corporate",
                test_mms_id,
            )
            creator2 = (
                str(secondCorporateAuthor)
                + "~ "
                + str(secondCorporateAuthorRelator)
                + "\n"
            )
        ...
        if re.search(r"[Ee]lectronic", gfSegment.iloc[x]["Format"]):
            format = re.sub(
                r"^.*?([Ee]lectronic\s[A-Za-z- ]+)", r"\1", gfSegment.iloc[x]["Format"]
            )
            print(format)
        publicationInfo = parsePublication(
            firstPlacePublished,
            firstPublisher,
            firstPublishedYear,
            secondPlacePublished,
            secondPublisher,
            secondPublishedYear,
        )
